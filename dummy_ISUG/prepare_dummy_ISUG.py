...
# : user must have setup the keys between hyperion and winterfell :#
ssh_winterfell = 'ssh django@10.85.40.177'
cat_regex = re.compile(r'(\w+)(?=:(\d{5,6}))')
dir = 'isu_cgr'
if options.branch.startswith('cat'):
    dir = 'isu'
vevak_url = 'http://vevak.dev.cic.nsn-rdnet.net/' + \
    dir + '/rev_db_cache_' + options.branch + '.txt'
homepath = os.environ['HOME']
# :this can be customized according to the user's needs :#
working_copy = '/scratch/' + os.environ['USER'] + '/git_trunk'
ipc_file = os.environ['TOOLS_ROOT'] + \
    '/troubleshoot/msgmon/conf/ipc_encoding_scheme.conf'
supported_file = os.environ['LIB_ROOT'] + \
    '/libisu_mc/manage_gen/supported_versions.conf'
buildbot_url = 'http://fra.dev.cic.nsn-rdnet.net/' + \
    options.branch + '/one_box_per_builder'
ipc_message_db = ['LIBSYNC_HA_UPDATE',
                  'PEM_CTRL_SC_PEER_DOWN_INDICATION', 'CDR_COLLECTOR_HA_CTRL_SWO_NOTIFY']
...
def ipc_message_gen(file_object):
    ...
    for message in ipc_message_db:
        x = filter(lambda line: re.match(
            'M=' + message + ' ', line), file_object.readlines())
...
def edit_ipc(message_to_modify):
    ...
    for line in fileinput.input(ipc_file, inplace=True):
        if re.match(message_to_modify.split()[0] + ' ', line):
            log.info('Match found in ipc scheme for %s.Change that' %
                     message_to_modify.split()[0])
...
def set_ISU_flag(line):
    ...
    log.info('Old value for ISU flag of %s is %s' %
             (' '.join(line.split()), old_value))
...
def enter_winterfell():
    ...
    django.expect_exact('$')
    c = pexpect.spawn('scp django@10.85.40.177:/home/django/lvnproject/Weblvn/my_media/svnhistory/' +
...
if __name__ == '__main__':
    ...
    try:
        ...
        log.info('FIRST COMMIT WILL HAPPEN NOW.Green revision currently on trunk is %s and message to modify:%s: ' % (
            green_rev, ' '.join(message.split())))
        #: first local commit to git :#
        commit_message(
            'REF:dummy commit to ipc scheme for dummy ISUG' + '\n' + 'DESCR:\n', [ipc_file])
        ...
        log.info(
            'SECOND COMMIT WILL HAPPEN NOW.Revert of ipc scheme and commit supported versions')
            ...
            if stashes and options.branch in [pat.search(line).group(1) for line in stashes] and options.branch == [cur.split()[1] for cur in all_branches.splitlines() if '*' in cur][0]:
                ...
                for line in stashes:
                    ...
                    if branch and branch == options.branch:
                        log.warning('Found stash %s for %s' %
                                    (line, options.branch))
                        stash_rev = re.match('stash@{(\d+)}', line).group(1)
                        git.stash('pop stash@{' + stash_rev + '}')
                        log.warning('Pop up this stash %s back to %s' %
                                    (line, options.branch))
                        break
                    else:
                        pass
